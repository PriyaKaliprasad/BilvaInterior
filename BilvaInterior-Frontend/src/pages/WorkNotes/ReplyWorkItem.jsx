...
const ReplyWorkItem = ({ taskId, currentUserRole, onBack, onUpdate }) => {
    ...
    const handleSubmit = async () => {
        ...
        // Use the passed Role, or default to Employee if missing
        const roleToSend = currentUserRole || "Employee";

        console.log("Submitting Reply:", { taskId, status, roleToSend }); // Debug log
        ...
        try {
            ...
            if (res.ok) {
                // Success message depends on who replied
                const successText = roleToSend === "Admin" ? "Reply sent!" : "Reply sent to Admin!";
                setMessage({ text: `${successText} Redirecting...`, type: "success" });
                ...
                setMessage({ text: "Error submitting reply. Check console.", type: "danger" });
            }
        } catch (err) {
            console.error("Submit error:", err);
            setMessage({ text: "Something went wrong.", type: "danger" });
    ...
    return (
        <div className="card shadow-sm p-3">
            ...
            <div className="mb-3">
                <label className="form-label fw-bold" style={{ fontSize: "1.1rem" }}>Add Reply / Comments</label>
                <textarea
                    className="form-control"
                    placeholder={`Enter your update...`}
                    rows="3"
            ...
            <div className="mb-3">
                <label className="form-label fw-bold" style={{ fontSize: "1.1rem" }}>Attach File (Optional)</label>
                <input
                    type="file"
                    className="form-control"
                    onChange={(e) => setSelectedFile(e.target.files[0])}
            ...
            <div className="mb-3">
                <button className="btn btn-primary px-4" onClick={handleSubmit}>Send Reply</button>
            </div>
            ...
            <div className="chat-container p-3 bg-white rounded border" style={{ maxHeight: "400px", overflowY: "auto" }}>
                {replies.length === 0 ? (
                    ...
                    replies.map((reply) => {
                        const isByAdmin = reply.repliedBy?.toLowerCase() === "admin";
                        const bubbleClass = isByAdmin ? "bg-light text-dark border" : "bg-white text-dark border border-secondary";
                        const alignmentClass = isByAdmin ? "align-self-start text-start me-auto" : "align-self-end text-end ms-auto";

                        return (
                            <div key={reply.replyId} className="d-flex flex-column mb-3" style={{ maxWidth: "100%" }}>
                                <div className={`p-3 rounded shadow-sm ${bubbleClass} ${alignmentClass}`} style={{ maxWidth: "75%", minWidth: "200px" }}>
                                    <div className="d-flex justify-content-between border-bottom border-secondary pb-1 mb-2 opacity-75">
                                        <strong style={{ fontSize: "0.9em" }}>{reply.repliedBy || "Unknown"}</strong>
                                        <small style={{ fontSize: "0.75em", marginLeft: "10px" }}>{formatDate(reply.createdAt)}</small>
                                    </div>
                                    <div className="mb-1">{reply.comment}</div>
                                    <div className="mt-2">
                                        <span className="badge bg-secondary text-white" style={{ fontSize: "0.7em" }}>Status: {reply.status}</span>
                                    </div>
                                    {reply.filePath && (
                                        <div className="mt-2 pt-1 border-top border-secondary">
                                            <a href={`${import.meta.env.VITE_API_BASE_URL}${reply.filePath}`} target="_blank" rel="noopener noreferrer" className="text-primary text-decoration-underline" style={{ fontSize: "0.9em" }}>
                                                View Attachment 📎
                                            </a>
                                        </div>
                                    )}
